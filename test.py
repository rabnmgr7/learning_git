...
def perform_login(session, username, password):
    ...
    headers1 = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/118.0.0.0 Safari/537.36',
        'Referer': 'http://192.168.1.2/login_security.html'
    }
    css_url = 'http://192.168.1.2/css/style.css'
    response = session.get(css_url, headers=headers1)
    time.sleep(1)
   # print("Redirection URL = ", redirect_url)
    ...
    headers3 = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/118.0.0.0 Safari/537.36',
        'Referer': 'http://192.168.1.2/status.html'
    }
    headers4 = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/118.0.0.0 Safari/537.36',
        'Referer': 'http://192.168.1.2/navigation-status.html'
    }
    headers5 = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/118.0.0.0 Safari/537.36',
        'Referer': 'http://192.168.1.2/status/status_deviceinfo.html'        
    }
    if response.status_code == 200:
        ...
        response = session.get(css_url, headers=headers3, allow_redirects=False)
        html_content += response.text

        general_url = 'http://192.168.1.2/js/general.js'
        response = session.get(general_url, headers=headers4, allow_redirects=False)
        html_content += response.text

        logo_url = 'http://192.168.1.2/Images/Logo.gif'
        response = session.get(logo_url, headers=headers3, allow_redirects=False)
        html_content += response.text

        tb3_url = 'http://192.168.1.2/Images/tb3.gif'
        response = session.get(tb3_url, headers=headers3, allow_redirects=False)
        html_content += response.text

        response = session.get(css_url, headers=headers4, allow_redirects=False)
        html_content += response.text
...
#############authentication is not replicated###############